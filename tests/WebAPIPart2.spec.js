...
// For more complex login situations you can login once
// then have playwright get local storage, session storage, cookies, etc
// and use those values for other tests 
// it will store it in json form
// inject that json file into the browser for the next tests
// NOTE: difference between browser and page. A page is the tab, the browser is all the tabs in the window/browser
let webContext;
...
test.beforeAll(async ({ browser }) => {
  ...
  // Create json file with session and cookie info in it
  await context.storageState({ path: 'state.json' });


  // open new browser context and inject JSON session/cookie into it
  webContext = await browser.newContext({ storageState: 'state.json' });

});


test('API Test 1 - Using imported JSON state', async () => {
...
test('API Test 2 - Using imported JSON state', async () => {
  const page = await webContext.newPage();
  await page.goto("https://rahulshettyacademy.com/client/");
